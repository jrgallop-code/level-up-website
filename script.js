...
const appDemoStyles = `
  ...
  .app-demo-copy{max-width:650px}.app-demo-copy h2{font-size:clamp(48px,6.2vw,82px);line-height:.94;letter-spacing:-.06em;margin:16px 0 24px}.app-demo-copy h2 em{color:var(--red);font-style:normal}.app-demo-copy>p{max-width:610px;margin:0;color:#9a9996;font-size:15px;line-height:1.75}
  .app-demo-points{display:grid;grid-template-columns:1fr 1fr;gap:1px;margin-top:30px;background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.10)}.app-demo-point{padding:18px;background:#0b0b0c;display:grid;gap:6px;min-height:90px}.app-demo-point strong{font-size:13px;color:#fff}.app-demo-point span{font-size:11px;line-height:1.5;color:#777673}
  .app-demo-note{margin-top:18px;display:flex;align-items:center;gap:9px;color:#777673;font-size:10px;letter-spacing:.02em}.app-demo-note i{width:7px;height:7px;border-radius:50%;background:var(--red);box-shadow:0 0 16px rgba(241,58,48,.75);animation:app-demo-pulse 1.8s ease-in-out infinite}
  .app-demo-visual{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:16px 0}.app-demo-glow{position:absolute;left:50%;top:48%;width:560px;height:560px;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(241,58,48,.18),rgba(241,58,48,.05) 38%,transparent 69%);filter:blur(18px);pointer-events:none}
  .app-demo-phone{position:relative;width:min(430px,94vw);padding:8px;border-radius:58px;background:linear-gradient(145deg,#3d3d42 0%,#101012 12%,#020203 86%,#333338 100%);box-shadow:0 38px 90px rgba(0,0,0,.68),0 0 0 1px rgba(255,255,255,.15),inset 0 0 0 1px rgba(255,255,255,.07),0 0 95px rgba(241,58,48,.09);transform:none;transition:transform .3s ease}.app-demo-phone:hover{transform:translateY(-3px)}
  .app-demo-side-button{position:absolute;left:-5px;width:5px;border-radius:5px 0 0 5px;background:linear-gradient(#303035,#111114);box-shadow:-1px 0 0 rgba(255,255,255,.12)}.app-demo-side-button-one{top:155px;height:70px}.app-demo-side-button-two{top:242px;height:112px}
  .app-demo-camera{position:absolute;z-index:6;top:18px;left:50%;transform:translateX(-50%);width:31%;height:27px;border-radius:99px;background:#030303;box-shadow:inset 0 0 0 1px rgba(255,255,255,.04)}.app-demo-camera span{position:absolute;right:12px;top:9px;width:8px;height:8px;border-radius:50%;background:#090b11;box-shadow:inset 0 0 0 2px #11172a,0 0 5px rgba(62,94,164,.25)}
  .app-demo-screen{position:relative;width:100%;aspect-ratio:944/2048;overflow:hidden;border-radius:50px;background:#111114;box-shadow:inset 0 0 0 1px rgba(255,255,255,.06)}.app-demo-screen:after{content:"";position:absolute;z-index:4;inset:0;pointer-events:none;background:linear-gradient(115deg,rgba(255,255,255,.025),transparent 18% 82%,rgba(255,255,255,.012));border-radius:inherit}
  .app-demo-screen img{position:absolute;z-index:2;inset:0;width:100%;height:100%;display:block;object-fit:contain;object-position:center;background:#111114;opacity:0;transition:opacity .38s ease}.app-demo-phone.is-loaded .app-demo-screen img{opacity:1}.app-demo-phone.is-loaded .app-demo-loading{opacity:0;visibility:hidden}
  .app-demo-loading{position:absolute;z-index:1;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;background:#111114;transition:opacity .35s ease,visibility .35s ease}.app-demo-loading b{font-size:15px;letter-spacing:.15em;color:#fff}.app-demo-loading span{font-size:10px;color:#727176}
  .app-demo-caption{position:relative;z-index:2;width:min(430px,94vw);display:flex;justify-content:space-between;gap:12px;padding:13px 10px 0}.app-demo-caption strong{font-size:9px;letter-spacing:.12em;color:#fff}.app-demo-caption span{font-size:9px;color:#777673}
...
function mountAppDemo() {
  ...
  const loadTour = async () => {
    if (started) return;
    started = true;
    const parts = Array.from({ length: 10 }, (_, index) => `assets/app-demo/part-${String(index).padStart(2, '0')}.txt?v=app-tour-2`);
    try {
